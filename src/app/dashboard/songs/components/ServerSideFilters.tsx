...
export function ServerSideFilters({
	songs,
	filterOptions,
	...
	const [songKey, setSongKey] = useState(searchParams.get('key') || 'All');
	...
	const uniqueLevels = filterOptions
		...
		: [
				...
				...Array.from(
					new Set(
						songs.map((song: Song) => song.level).filter(Boolean) as string[]
					)
				),
		  ];
	...
	const uniqueKeys = filterOptions
		...
		: [
				...
				...Array.from(
					new Set(
						songs.map((song: Song) => song.key).filter(Boolean) as string[]
					)
				),
		  ];
	...
	const uniqueAuthors = filterOptions
		...
		: [
				...
				...Array.from(
					new Set(
						songs.map((song: Song) => song.author).filter(Boolean) as string[]
					)
				),
		  ];
	...
	const hasActiveFilters = Boolean(
		searchTerm || level !== 'All' || songKey !== 'All' || author !== 'All'
	);
