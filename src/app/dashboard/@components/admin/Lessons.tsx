      ...
      {lessons.map((lesson) => (
        <div key={lesson.name} className={`rounded-xl shadow p-5 ${lesson.color} flex flex-col gap-2`}>
          ...
          <div className="flex items-center gap-2 text-xs text-gray-400">
            <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded font-semibold">{lesson.time}</span>
            <button className="text-blue-600 hover:underline ml-auto">View Details</button>
            <button className="text-gray-500 hover:underline">Reschedule</button>
          </div>
        </div>
      ))}
    </div>
